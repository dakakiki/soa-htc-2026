...
- **`legacy:import-countries`** — 96 legacy redova → **95** naših; reconcile postojećeg seed-a po imenu (+alias „N.Macedonia"); `countries.code` proširen `char(2)→char(3)` za legacy short (2–3 znaka). **Tajland je u legacy bazi dva puta** (43 „Thailand PHI" i 99 „Thailand ICE" — dve partnerske organizacije, jedna zemlja, ISO 764): fold je deklarisan u `App\Domain\Migration\LegacyCountries::MERGES`, importer preskoči red 99, a svi importeri koji nose legacy `country_id` razrešavaju ga preko `LegacyCountries::map()` — inače bi 54 venue-a završila u karantinu kao „country not mapped". Već uvezenu bazu sređuje `countries:merge-legacy-duplicates` (ima `--dry-run`). Vidi **ADR-0041**.
