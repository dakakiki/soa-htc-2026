...
- **NIJE rađeno (svesno, van dogovorenog obima):** `PageLayout` zone i **vezivanje menija za lokacije** (`public.header` / `public.footer` / `admin.top` / `admin.right_sidebar`) — meniji se sada prave i uređuju, ali `PublicLayout.vue` još crta hardkodovan niz umesto da traži meni po slug-u. Uz to, otvaranjem javnog dela **postaje vidljiv beli logo na belom javnom zaglavlju** (ranije odloženo pitanje, sada stvarno).
