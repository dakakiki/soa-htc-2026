...
export default {
    ...
    notifications: {
        ...
        header: 'Header',
        headerHint: 'The masthead at the top of every e-mail: a logo with the name beside it.',
        logo: 'Logo',
        chooseImage: 'Choose an image',
        logoHint: 'PNG, JPG or WebP — not SVG. Gmail and Outlook draw no vector image, so an SVG logo leaves the masthead empty.',
        logoBorrowed: 'No image here, so the e-mail uses the logo from Settings → Theme.',
        logoNone: 'No image here and none in Settings → Theme that an e-mail can draw, so the masthead shows the name alone.',
        headerText: 'Text beside the logo',
        headerTextHint: 'Left empty, the site title is used.',

        body: 'Body',
        bodyHint: 'The lines around whatever the message itself says. What goes between them is written when the message is sent.',
        greeting: 'Greeting',
        // 🪤 `{'{'}name{'}'}` and not `{name}`: vue-i18n reads a bare brace as a
        // parameter and would print nothing where the token should be.
        greetingHint: "{'{name}'} is replaced with the recipient’s name.",
        signoff: 'Sign-off',
        signoffHint: "{'{site}'} is replaced with the site title. Line breaks are kept.",

        footer: 'Footer',
        footerHint: 'The foot of every e-mail, under the brand rule.',
        footerWeb: 'Web address',
        footerEmail: 'E-mail address',
        footerText: 'Text',
        footerTextHint: 'Left empty, the e-mail uses the website’s own footer text, so the two can never say different things. Fill this in only to make the letter say something the page does not.',
    },
    layout: {
        title: 'Layout',
        noMenu: '— no menu —',
    ...
    registration: {
        ...
        attempts: {
            ...
            colScore: 'Score',
            reset: 'Reset result',
            modalTitle: 'Reset this result?',
            modalBody: 'The result for "{test}" stops counting and the student can sit this exam again. It is kept with the reason below, so it can still be looked up.',
            reason: 'Reason (required)',
            reasonPlaceholder: 'Why is this result being reset?',
    ...
    test: {
        ...
        addNote: 'Add note',
        note: 'Note',
        noteHeading: 'Note between the questions',
        emptyNote: '(empty note)',
        addNoteHint: 'A heading or instruction between the questions. Never numbered, never marked.',
        notePlaceholder: 'TASK 1 Read the text and answer the questions.',
        alreadyAdded: 'Already added',
        status: 'Status',
        statusActive: 'On',
        statusInactive: 'Off',
    ...
    currentAction: {
        ...
        subtitle: 'Who is sitting an exam right now. The counts refresh every ten seconds, and pause while this tab is in the background.',
        running: 'Sitting now',
        submittedRecently: 'Handed in, last {n} min',
        overdue: 'Past the deadline',
        // \U0001FAA4 Says what the number IS, because "past the deadline" reads like a
    ...
    userLog: {
        ...
        to: 'To',
        search: 'Search',
        searchPlaceholder: 'Name, address or IP\u2026',
