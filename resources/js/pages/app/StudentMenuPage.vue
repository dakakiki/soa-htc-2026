 ...
 *
 * Three doors, and every one of them leads to the same identification screen
 * with a different stream behind it ({@see StudentAccessFormPage}) — which is
 * why the round is not named here. Which round a candidate sits follows from
 * their level and their country, and none of that is known until the three
 * details have been given: a heading promising "Round 15" to whoever opens the
 * app would be wrong for about half the countries reading it (ADR-0077).
 *
...
<template>
    <div class="flex min-h-screen flex-col bg-brand-palette-4 text-white">
        <div class="mx-auto flex w-full max-w-[26rem] flex-1 flex-col px-6 pb-9 pt-8 sm:pt-12">
            <!-- The way back to the other card, and the only chrome this screen
                 carries. A child who tapped Student by mistake needs it. -->
            <div class="flex items-center gap-3">
                ...
                <span class="text-base font-semibold tracking-[-0.01em]">{{ $t('public.app.student') }}</span>
            </div>
            ...
            <div class="mt-5 grid gap-3">
                <!-- The exam, and the only row that is answered with a password
                     read out in the room. It carries the weight for that reason. -->
                <RouterLink
                    :to="{ name: 'student.access.form', params: { mode: 'competition' } }"
                    :class="row"
                ...
                <RouterLink
                    :to="{ name: 'student.access.form', params: { mode: 'sample' } }"
                    :class="row"
                ...
                <RouterLink
                    :to="{ name: 'student.access.form', params: { mode: 'results' } }"
                    :class="row"
                    class="border border-white/20 bg-white/5 hover:bg-white/10"
                >
                    <span>
                        <span class="block text-[1.02rem] font-semibold tracking-[-0.01em]">{{ $t('public.app.results') }}</span>
                        <!-- `<i18n-t>` rather than `$t`: the two words that name
                             the streams have to be elements to be set apart, and
                             both are named because the screen behind this row
                             shows both blocks, in that order (ADR-0054). -->
