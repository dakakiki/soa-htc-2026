<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    public function up(): void
    {
        Schema::table('schools', function (Blueprint $table) {
            $table->string('city')->nullable()->after('region_id');
            $table->string('address')->nullable()->after('city');
            $table->string('phone', 100)->nullable()->after('address');
            $table->string('email')->nullable()->after('phone');
            $table->string('image_path')->nullable()->after('email');
            $table->unsignedSmallInteger('hours_eng_per_week')->nullable()->after('image_path');
            $table->unsignedSmallInteger('invigilators_count')->nullable()->after('hours_eng_per_week');
            $table->string('school_type', 20)->nullable()->after('invigilators_count');
        });
    }

    public function down(): void
    {
        Schema::table('schools', function (Blueprint $table) {
            $table->dropColumn([
                'city', 'address', 'phone', 'email', 'image_path',
                'hours_eng_per_week', 'invigilators_count', 'school_type',
            ]);
        });
    }
};
